...
+ (void)restoreAudioSession
{
	...
	// After restoreAudioSession function
	NSLog( @"=== AFTER FUNCTION END: %@", [[AVAudioSession sharedInstance] category] );
	NSLog( @"=== AFTER FUNCTION END: %lu", (unsigned long) [[AVAudioSession sharedInstance] categoryOptions] );
	NSLog( @"=== AFTER FUNCTION END: %@", [[AVAudioSession sharedInstance] mode] );
}
